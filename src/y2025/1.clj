...
(defn part1 [input]
  ...
  (loop [pos [50]
    ...
    (let [m (first moves)]
      (if m
        (recur (conj pos (mod ((if (= "L" (first m)) - +) (last pos) (second m)) 100)) (rest moves))
        pos)))
  (filter zero?)
  count))
    ...
    (let [m (first moves)]
      ...
      (if m
        (let [n (second m)
              t (quot n 100)
              r (rem n 100)
              np ((if (= "L" (first m)) - +) (last pos) r)]
          ;(println c np)
          (if (or (zero? np) (and (not (zero? (last pos))) (or (<= 100 np) (> 0 np))))
            (recur (conj pos (mod np 100)) (rest moves) (+ c t 1))
            (recur (conj pos (mod np 100)) (rest moves) (+ c t))))
        c))))
